...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/whisper-appliance.log'),
...
class EnhancedWhisperModelManager:
    ...
    async def load_model_with_limits(self, model_name: str):
        ...
        if required_gb > available_gb:
            raise HTTPException(
                status_code=507, 
                detail=f"Insufficient RAM: {required_gb:.1f}GB required, {available_gb:.1f}GB available"
            )
        ...
        if required_gb > state.max_ram_usage:
            raise HTTPException(
                status_code=507,
                detail=f"Model exceeds RAM limit: {required_gb:.1f}GB > {state.max_ram_usage:.1f}GB"
            )
        ...
        try:
            ...
            try:
                ...
                state.current_model = WhisperModel(
                    model_name, 
                    device="cpu", 
                    compute_type="int8"
                )
